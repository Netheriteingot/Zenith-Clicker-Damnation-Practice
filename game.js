class CardGame {
    ...
    drawInitialCards(playerId) {
        ...
        let drawPool = availableCards.filter(card =>
        ...
        player.hand = [];
        while (player.hand.length < player.cardLimit && drawPool.length > 0) {
            const randomIndex = Math.floor(Math.random() * drawPool.length);
            player.hand.push(drawPool.splice(randomIndex, 1)[0]);
        }
        return player.hand;
    }
    ...
    reselectCharacter(playerId, characterId, newCharacterId) {
        ...
        this.updatePlayerCardLimit(playerId); // <-- Ensure this is called!
    ...
    startTurn() {
        ...
        while (player.hand.length < player.cardLimit) {
            const drawn = this.drawCards(this.currentPlayer, 1);
    ...
    parseData(type, val) {
        ...
        if (type === 'game_action') {
            ...
            if (action.type === 'use_card' && action.player !== this.currentPlayer) {
                ...
                if (player?.hand.includes(action.card)) {
                    if (action.card >= 1 && action.card <= 20 && action.target && typeof action.target.idx === "number") {
                        let targetPlayer;
                        if (action.target.side === "player") targetPlayer = player;
                        else if (action.target.side === "opponent") targetPlayer = opponent;
                        else return;
                        if (targetPlayer === player && targetPlayer.characters[action.target.idx]) {
                            targetPlayer.characters[action.target.idx].health += 1;
                        }
                    }
                    player.hand = player.hand.filter(c => c !== action.card);
                    player.discardPile.push(action.card);
                }
                // Always deselect and update UI
                if (typeof window !== "undefined" && typeof updateGameState === "function") {
                    if (typeof selectedCharacterIndex !== "undefined") selectedCharacterIndex = null;
                    updateGameState(this.getGameState());
                }
                return;
            }
            ...
            if (action.type === 'character_reselected') {
                ...
                if (charIndex !== -1) {
                    ...
                    // FIX: Always use updatePlayerCardLimit
                    this.updatePlayerCardLimit(action.player);
                }
                return;
            }
            ...
            if (action.type === 'turn_end') {
                ...
                // Do NOT startTurn() here! The host will send a turn_start event next.
                return;
            }
        ...
        if (this.gameState === 'character_selection') {
            if (type === 'swap') {
                ...
                if (this.players[this.currentPlayer]?.characters.length > 0 &&
                    ...
                    this.updatePlayerCardLimit('player1');
                    this.updatePlayerCardLimit('player2');
            ...
            if (type === 'reselect') {
                ...
                if (charIndex !== -1 && newChar) {
                    player.characters[charIndex] = newChar;
                    // FIX: Always use updatePlayerCardLimit
                    this.updatePlayerCardLimit(this.currentPlayer);
                    this.sendAction({
                        type: 'character_reselected',
                        player: this.currentPlayer,
        ...
        // In parseData(), in 'game_action' block for 'use_card', update as follows:
        if (action.type === 'use_card' && action.player !== this.currentPlayer) {
            const player = this.players[action.player];
            if (player?.hand.includes(action.card)) {
                // Healing logic for remote
                if (action.card >= 1 && action.card <= 20 && action.target && typeof action.target.characterIndex === "number") {
                    // Only heal if target is player's own character
                    if (player.characters[action.target.characterIndex]) {
                        player.characters[action.target.characterIndex].health += 1;
                    }
                }
                player.hand = player.hand.filter(c => c !== action.card);
                player.discardPile.push(action.card);
        ...
        if (action.type === 'end_turn' && this.host === 1) {
            this.endTurn();
            return;
        }
    ...
    // In handleUseCard(), update as follows:
    handleUseCard(playerId, card, target) {
        ...
        // Healing logic: cards 1~20, only your own characters
        if (card >= 1 && card <= 20 && target && typeof target.idx === "number") {
            let targetPlayer;
            if (target.side === "player") targetPlayer = player;
            else if (target.side === "opponent") targetPlayer = opponent;
            else return;

            if (targetPlayer === player && targetPlayer.characters[target.idx]) {
                targetPlayer.characters[target.idx].health += 1;
            } else {
                // Invalid target for healing card
                return;
            }
        }

        // (Put other card logic here as needed...)
